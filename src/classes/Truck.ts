...
class Truck extends Vehicle implements AbleToTow {
  ...
  tow(vehicle: Truck | Motorbike | Car): void {
    let make;
    let model;
    // Gets the make an model of the vehicle if it exists
    vehicle.make ? make = vehicle.make : make = '';
    vehicle.model ? model = vehicle.model : model = '';
    ...
    if (vehicle.weight <= this.towingCapacity) {
      ...
      console.log(`This ${make} ${model}'s weight exceeds the towing capacity of the truck and, unfortunately, cannot be towed.`)
    }
  }
